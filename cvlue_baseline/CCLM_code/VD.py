...
from models.model_vd import VDModel
...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    metric_logger = utils.MetricLogger(delimiter="  ")
    header = 'Generate VD test result:'
    print_freq = 50
    ...
    if args.evaluate:
        ...
        for epoch in range(start_epoch, max_epoch):
            ...
            if epoch == 0 or epoch == 9 or epoch == 4:
                ...
                result = collect_result(vd_result, 'vd_valid_epoch%d' % epoch, local_wdir=args.result_dir, hdfs_wdir=args.output_hdfs,
                                         write_to_hdfs=world_size > 8, save_result=False)
