...
def get_dist_launch(args):  # some examples
    if args.dist == 'all':  # use all nodes
       ...
       gpus = args.dist[3:]
       tmp_gpus = args.dist[3:].split(',')
       return "CUDA_VISIBLE_DEVICES={:} WORLD_SIZE={:} python3 -m torch.distributed.launch --nproc_per_node={:} " \
    "--nnodes=1 --master_port={:}".format(gpus, len(tmp_gpus), len(tmp_gpus), args.master_port)
...
def run_vqa_ch(args, load_vqa_pretrain=False):
    dist_launch = get_dist_launch(args)
    ...
    os.system(f"{dist_launch} "
              f"--use_env VQA_ch.py --config {args.config} {'--load_vqa_pretrain' if load_vqa_pretrain else ''}"
              f"{f'--output_hdfs {args.output_hdfs}' if len(args.output_hdfs) else ''} --output_dir {args.output_dir} "
              f"--bs {args.bs} --seed {args.seed} --checkpoint {args.checkpoint} {'--evaluate' if args.evaluate else ''} "
              f"{'--load_vqa_pretrain --fewshot ' + args.fewshot if args.fewshot else ''} --lr {args.lr}")
...
def run_vd(args, load_vqa_pretrain=False):
    dist_launch = get_dist_launch(args)

    # assert os.path.exists("images/gqa")

    print("### Training VD", flush=True)
    # args.config = f"configs/{args.model}/GQA_fewshot.yaml" if args.fewshot else f"configs/{args.model}/GQA.yaml"

    os.system(f"{dist_launch} "
              f"--use_env VD.py --config {args.config} {'--load_vqa_pretrain' if load_vqa_pretrain else ''}"
              f"{f'--output_hdfs {args.output_hdfs}' if len(args.output_hdfs) else ''} --output_dir {args.output_dir} "
              f"--bs {args.bs} --seed {args.seed} --checkpoint {args.checkpoint} {'--evaluate' if args.evaluate else ''} "
              f"{'--load_vqa_pretrain --fewshot ' + args.fewshot if args.fewshot else ''} --lr {args.lr}")
...
def run_vg_ch(args):
    ...
    print("### Training VG_CH", flush=True)
    # if not os.path.exists(args.config): args.config = f"configs/{args.model}/Grounding_bbox.yaml"

    os.system(f"{dist_launch} "
              f"--use_env Grounding_bbox_ch.py --config {args.config} "
              f"--output_dir {args.output_dir} {f'--output_hdfs {args.output_hdfs}' if len(args.output_hdfs) else ''} "
              f"--bs {args.bs} --seed {args.seed} --checkpoint {args.checkpoint} "
              f"{'--evaluate' if args.evaluate else ''} ")
...
def run(args):
    ...
    elif args.task == 'itr_coco_test':
        run_itr_coco_test(args)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--accumulate_steps', type=int)
