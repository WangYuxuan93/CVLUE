...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for image, img_id, img_path in tqdm(data_loader):
...
@torch.no_grad()
def itm_result(data_loader, scores_i2t, scores_t2i):
    # Images->Text

    texts = data_loader.dataset.text   
    text_id2txt = {i:text for i,text in enumerate(texts)}


    image_id2img = {}
    i = 0
    for image, image_id, image_paths in(data_loader):
        for image_path in image_paths:
            image_id2img[i] = image_path
            i += 1
    ...
    for index, score in enumerate(scores_t2i):
        ...
        tmp_text = text_id2txt[index]
        inds_list = inds.tolist()

        top_15_ids = inds_list[:15]

        tr_result[tmp_text] = []

        for top_id in top_15_ids:
            tr_result[tmp_text].append(image_id2img[int(top_id)])
    ...
    for index, score in enumerate(scores_i2t):
        ...
        for top_id in top_15_ids:
            ir_result[tmp_img].append(text_id2txt[int(top_id)])

    i2t_result_path = os.path.join(args.output_dir, 'i2t_result.json')
    t2i_result_path = os.path.join(args.output_dir, 't2i_result.json')

    with open(i2t_result_path, "w", encoding="utf-8") as f:
        json.dump(ir_result,f,ensure_ascii=False,indent=1)
    
    with open(t2i_result_path, "w", encoding="utf-8") as f:
        json.dump(tr_result,f,ensure_ascii=False,indent=1)
...
def main(args, config):
    ...
    if args.evaluate:
         for language, [val_loader, test_loader] in val_test_loader_set.items():
            ...
            itm_result(test_loader, score_test_i2t, score_test_t2i)
            # itm_result(val_loader, score_val_i2t, score_val_t2i)
