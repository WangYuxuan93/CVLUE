...
from dataset.vqa_dataset import xgqa_dataset, xgqa_ch_dataset, next_qa_mc_dataset
...
def create_dataset(dataset, config, evaluate=False):
    ...
    if dataset == 'pretrain':
        ...
        for language, (rpath, ans_rpath) in config['test_file'].items():
            test_dataset_dict[language] = xgqa_dataset(rpath, test_transform, config['vqa_root'], split='test', answer_list=config['answer_list'],
                                                      text_encoder=config['text_encoder'])

        return train_dataset, valid_dataset, test_dataset_dict
    
    elif dataset == 'xgqa_ch':
        train_dataset = xgqa_ch_dataset(config['train_file'], train_transform_wohflip, config['vqa_root'],
                                    split='train', text_encoder=config['text_encoder'])


        valid_dataset = xgqa_ch_dataset(config['valid_file'], test_transform, config['vqa_root'],
                                    split='valid', answer_list=config['answer_list'],
                                    text_encoder=config['text_encoder'])

        test_dataset_dict = {}
        for language, (rpath, ans_rpath) in config['test_file'].items():
            test_dataset_dict[language] = xgqa_ch_dataset(rpath, test_transform, config['vqa_root'], split='test', answer_list=config['answer_list'],
                                                      text_encoder=config['text_encoder'])
