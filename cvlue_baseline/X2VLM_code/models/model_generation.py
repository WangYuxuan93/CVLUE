...
class XVLMPlusForVQA(XVLMPlusBase):
    ...
    def load_pretrained(self, ckpt_rpath, config, is_eval=False):
        if is_eval:
            ...
            state_dict = load_pretrained(self, ckpt_rpath, config, load_text=True)
            ...
            for key in list(state_dict.keys()):
                ...
                if 'mlm_head' in key:
                    decoder_key = key.replace('mlm_head', 'text_decoder.cls')
                    state_dict[decoder_key] = state_dict[key]

                if 'embeddings' in key and 'position_ids' not in key:
                    decoder_key = key.replace('text_encoder', 'text_decoder.bert')
                    state_dict[decoder_key] = state_dict[key]
...
# copied from XVLMPlusForVQA, changed the decoding part
class XVLMPlusForVD(XVLMPlusBase):
    """
    Generative Model, but model the task as ranking at inference
    """
    def __init__(self, config, tied=False):
    ...
    def rank_answer_0(self, question_states, question_atts, answer_ids, answer_atts, k):

        num_ques = question_states.size(0)

        start_ids = answer_ids[0,0, 0].repeat(num_ques, 1)  # bos token
        ...
        topk_probs = []
        topk_ids = []
        for i in range(answer_first_token.size(0)):
            temp_answer_first_token = answer_first_token[i]
            temp_logits = logits[i]
            temp_prob_first_token = F.softmax(temp_logits,dim=0).index_select(dim=0,index=temp_answer_first_token)
            temp_topk_probs, temp_topk_ids = temp_prob_first_token.topk(k)
            topk_probs.append(temp_topk_probs)
            topk_ids.append(temp_topk_ids)
        ...
        input_atts = []
        for b, topk_id in enumerate(topk_ids):
            input_ids.append(answer_ids[b,:,:].index_select(dim=0, index=topk_id))
            input_atts.append(answer_atts[b,:,:].index_select(dim=0, index=topk_id))
        input_ids = torch.cat(input_ids, dim=0)
        input_atts = torch.cat(input_atts, dim=0)
        ...
        topk_probs = torch.stack(topk_probs)
        # topk_prob: first token probability
        topk_probs = topk_probs.view(-1, 1)
