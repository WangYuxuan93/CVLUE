...
def train(model, data_loader, optimizer, tokenizer, epoch, device, scheduler, config):
    ...
    accumulate_steps = int(config.get('accumulate_steps', 32))
...
def main(args, config):
    ...
    train_dataset, valid_dataset, test_dataset_dict = create_dataset('xgqa_ch', config)
    ...
    model = XVLMPlusForVQA(config=config, tied = True)
