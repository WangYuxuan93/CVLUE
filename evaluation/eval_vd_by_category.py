...
import jsonlines
from eval_vd import eval_vd_json, merge_vd
...
parser.add_argument("--output_path", default="vd_score_by_category.json", help="path to output score json file.")
...
if args.zh_key:
    gold_file = "test_VD.json"
    with open(gold_file, "r", encoding="utf-8") as fi:
        data = json.load(fi)
    id2quest = data["id2question"]
        ...
        if args.zh_key:
            ...
            for key_word in zh_keys:
                if "query" in item:
                    ...
                    question = id2quest[str(item["question"])]
                #print (question)
                #exit()
...
if args.zh_key:
    ...
    n_total, r1, r5, r10 = eval_vd_json(filter_file_output)
else:
    n_total, r1, r5, r10 = eval_vd_json(merged_file)
scores["overall"] = {
        "n_tot": n_total,
        "r1": r1, 
        "r5": r5, 
...
for cat in result_by_cat:
    ...
    n_tot, r1, r5, r10 = eval_vd_json(output_file)
    scores[cat] = {
        "n_tot": n_tot,
        "r1": r1, 
        "r5": r5, 
